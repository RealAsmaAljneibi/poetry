...
**Insight 1 — Multitask sharing is the headline architectural result.** The genre and emotion taxonomies share latent structure (Hikma poems carry Contemplation, Ghazal carries Delicate Love). Training a single AraPoemBERT encoder to simultaneously predict genre and emotion gives a 4.1× absolute genre F1 lift (0.132 → 0.543) at zero parameter increase. This is not a standard multitask gain — most multitask papers report small improvements. At this dataset scale, the emotion head provides enough co-regularisation to prevent the genre head from overfitting to the Hikma-dominated split. The t-SNE in Figure 1 shows why: Ghazal, Shajan, and Hikma occupy overlapping embedding space. Emotion supervision differentiates them by teaching the encoder that Ghazal carries Delicate Love, Shajan carries Sorrow, and Hikma carries Contemplation — distinctions that genre labels alone cannot enforce without the emotion signal.

**Insight 2 — ASR fine-tuning has a hard floor at ~3 K clips.** Seven LoRA runs covering every major axis of adaptation (data curriculum, architecture freezing, augmentation, optimizer, domain replay) all regressed below zero-shot WER=0.272. Run R4 (frozen encoder) proves the forgetting is decoder-level, not encoder-level — the seq2seq decoder is simultaneously a transducer and a language model, and ~3 K Nabati clips cannot recalibrate its language-modelling prior without overwriting it. The actionable threshold: do not fine-tune Whisper on dialectal data below ~30 K clips.

**Insight 3 — Self-supervised contrastive learning beats supervised on heavily imbalanced audio.** NT-Xent loss on unlabeled clips (SimCLR, 0.088 F1) outperforms end-to-end supervised training (CNN-R3c, 0.060 F1). Figure 5 shows why supervised fails: direct cross-entropy on 12 imbalanced classes collapses every class to predicting Disappointment. NT-Xent pushes all clips apart in representation space regardless of class frequency — a fundamentally more informative objective for this setting.

**What this enables — real-world deployment:** A heritage-archive curator uploads a community-recorded Nabati poem and receives structured JSON in ~2 s: genre label, emotion label, arousal level, and the five most similar archived poems. The partial-credit score provides an operationalizable confidence threshold: predictions landing in T1 (exact) or T2 (adjacent) can be auto-tagged without human review; the remaining ~14% are flagged for manual verification. This converts a ~2-hour-per-poem manual cataloguing workflow into an exception-handling workflow: at AED 240/poem (AED 120/hr × 2 hrs, UAE Arabic specialist mid-market rate), only the flagged 14% require human review — AED 240 × 0.14 ≈ AED 34/poem in residual human cost, compared to AED 240 fully manual.

**What this enables — concrete actionable recommendations:** (a) For any future Nabati labelling effort: use multitask from the start — the genre and emotion taxonomies are entangled, and jointly training on both gives a structurally better genre head at zero extra inference cost. (b) For Gulf dialect ASR: do not fine-tune Whisper on fewer than ~30 K clips. Collect a larger dialectal corpus first, or use a domain-anchor pre-training stage; at 3 K clips, zero-shot is the ceiling. (c) For audio classification on heavily imbalanced unlabelled corpora: run SimCLR (or any contrastive SSL) before any supervised head — the NT-Xent objective provides richer gradient signal than cross-entropy and requires no labels.
