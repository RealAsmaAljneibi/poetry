...
def run_tfidf_retrieval(
    ...
    vec = TfidfVectorizer(
        analyzer="char_wb", ngram_range=(2, 5), max_features=20_000, sublinear_tf=True
    )
...
def run_hybrid_retrieval(
    ...
    for query in queries:
        ...
        if a_vec is not None:
            ...
            audio_score_map = {
                int(idx): float(s) for s, idx in zip(a_scores[0], a_idxs[0]) if idx >= 0
            }
        else:
            audio_score_map = {}
        ...
        for _, ci in candidates[:k]:
            c_genre = (
                (retriever.records[ci].get("genre_en") or "").split("(")[0].strip()
            )
            is_relevant.append(c_genre == genre and genre != "")
        while len(is_relevant) < k:
            is_relevant.append(False)
...
def main() -> None:
    ...
    logger.info(
        f"Queries (test):     {len(test_queries)} poems (poem-level, aggregated)"
    )
    ...
    logger.info(
        f"Index coverage: {sum(1 for q in test_recs if record_id(q) in id_to_pos)} "
    ...
    logger.info("\n" + "═" * 70)
    logger.info("  RETRIEVAL ABLATION  (genre-relevance, leave-one-out, test split)")
    logger.info("═" * 70)
    logger.info(f"  {'Mode':<40s}  {'P@10':>6}  {'NDCG@10':>8}  {'MRR':>6}")
    logger.info("  " + "─" * 60)

    # Mode A: TF-IDF
    ranked_a = run_tfidf_retrieval(corpus, test_queries, EVAL_K)
    results.append(
        evaluate_ranked_lists(ranked_a, EVAL_K, "A: TF-IDF (keyword baseline)")
    )

    # Mode B: Text-only neural
    ranked_b = run_text_neural_retrieval(
        retriever, all_vecs, id_to_pos, test_queries, EVAL_K
    )
    results.append(evaluate_ranked_lists(ranked_b, EVAL_K, "B: AraPoemBERT text-only"))

    # Mode C: Hybrid
    ranked_c = run_hybrid_retrieval(
        retriever, all_vecs, id_to_pos, test_queries, EVAL_K
    )
    if ranked_c is not None:
        results.append(
            evaluate_ranked_lists(ranked_c, EVAL_K, "C: Hybrid text+audio (α=0.7)")
        )
    ...
    # ── Interpretation ─────────────────────────────────────────────────────────
    logger.info("\nInterpretation:")
    logger.info("  B > A  → neural embeddings outperform keyword matching")
    logger.info("  C > B  → adding audio embeddings further improves retrieval")
    logger.info("  C ≈ B  → text embeddings already capture the relevant signal")
    logger.info()
    ...
    try:
        ...
        ax.set_title(
            "Retrieval Ablation: Text-only vs Hybrid\n(higher = better; same test split)"
        )
        ax.set_ylim(0, 1.0)
