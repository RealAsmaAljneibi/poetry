...
def search_reranked(
    ...
    for embed_score, idx in zip(scores[0], idxs[0]):
        ...
        candidates.append(
            {**rec, "score": final_score, "_embed_score": float(embed_score)}
        )
...
def eval_queries(
    ...
    for q_rec in queries:
        ...
        # GradedNDCG@10
        gndcg = graded_ndcg_at_k(
            results,
            q_genre,
            q_emotion,
            k=k,
            genre_key="genre_en",
            emotion_key="emotion_text",
        )
        ...
        if raw_tags:
            ic = imagery_coherence_at_k(
                results, raw_tags, k=k, imagery_key="imagery_tags_en"
            )
            if ic == ic:  # not NaN
...
def main() -> None:
    ...
    logger.info(
        f"Baseline (embed only):  GradedNDCG@10={base_gndcg:.4f}  "
    ...
    logger.info(
        f"Grid search over {len(beta_grid)}×{len(gamma_grid)} β,γ combinations on val ..."
    )
    ...
    logger.info(
        f"Best val: β={best_beta}  γ={best_gamma}  GradedNDCG@10={best_val_gndcg:.4f}"
    )
    ...
    logger.info(
        f"Re-ranked (β={best_beta}, γ={best_gamma}):  "
    ...
    logger.info("=" * 60)
    logger.info(
        f"GradedNDCG@10: {base_gndcg:.4f} → {rerank_gndcg:.4f} (Δ={improvement:+.4f})"
    )
    logger.info(f"PoetDiversity@10: {base_div:.3f} → {rerank_div:.3f}")
    logger.info(f"ImageryCoherence@10: {base_img:.4f} → {rerank_img:.4f}")
    logger.info(
        f"Decision: {'ADOPT re-ranking' if adopted else 'REJECT — no meaningful gain'}"
    )
