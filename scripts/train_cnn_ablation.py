...
def _make_runs() -> dict[str, CNNAblationConfig]:
    ...
    runs[
        "CNN-R3c"
    ].description = "Loss ablation — Vanilla cross-entropy (no focal, no smoothing)"
    ...
    runs[
        "CNN-R4b"
    ].description = "Optimizer ablation — Adam (no weight decay correction)"
    ...
    runs[
        "CNN-R6c"
    ].description = "Architecture — Wider (2 blocks: 512→1024), larger linear"
    ...
    runs[
        "CNN-R7a"
    ].description = "Augmentation — No SpecAugment (baseline for ablation)"
    ...
    runs[
        "CNN-R7b"
    ].description = "Augmentation — Aggressive SpecAugment (4 time + 4 freq masks)"
    ...
    runs[
        "CNN-R8b"
    ].description = "Regularization — High dropout=0.5 + high weight_decay=1e-3"
    ...
    runs[
        "CNN-R9a"
    ].description = "Pooling — Adaptive MaxPool (takes peak activation per channel)"
    ...
    runs[
        "CNN-R9b"
    ].description = "Pooling — AdaptiveAvgPool(4) keeps 4 time steps before flatten"
    ...
    runs[
        "CNN-R10a"
    ].description = "Scheduler — OneCycleLR (fast ramp, aggressive cosine)"
    ...
    runs[
        "CNN-R13"
    ].description = "MixUp aug (α=0.4) — smooth label interpolation between samples"
    ...
    runs[
        "CNN-R14"
    ].description = "CutMix aug (α=1.0) — paste freq×time patch; proportional label mix"
    ...
    runs[
        "CNN-R15"
    ].description = "MixUp+CutMix combined — aggressive label-mixing regularisation"
    ...
    # ablation alone — it requires combining the winners.
    runs["CNN-R16"] = deepcopy(runs["CNN-R1"])
    runs["CNN-R16"].run_id = "CNN-R16"
    runs[
        "CNN-R16"
    ].description = "Best-combo: GELU+3-block+CE+dropout=0.1+AvgPool4+OneCycle+MixUp"
    ...
    runs["CNN-R16"].use_mixup = True  # MixUp (R13)
...
class AblationAudioDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(
            "AblationAudioDataset: {} samples from {}",
            len(self.samples),
            jsonl_path.name,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# 4b. MIXUP AND CUTMIX AUGMENTATION
#
...
def run_epoch(
    ...
    for batch in loader:
        ...
        if is_train and cfg.use_mixup and mel.size(0) > 1:
            mel, labels, labels_b, lam_mix = mixup_batch(
                mel, labels, cfg.mixup_alpha, device
            )
        elif is_train and cfg.use_cutmix and mel.size(0) > 1:
            mel, labels, labels_b, lam_mix = cutmix_batch(
                mel, labels, cfg.cutmix_alpha, device
            )

        with autocast(enabled=cfg.mixed_precision):  # Mixed precision
            logits = model(mel)
        ...
        if is_train:
            ...
            if step % cfg.grad_accum == 0:
                ...
                # LR scheduler step (per-step for onecycle)
...
def _save_grad_norm_curve(grad_norms: list[float], out_path: Path, run_id: str) -> None:
    """Gradient norm trace — detect explosion."""
    if not grad_norms:
        return
...
def train_one_run(cfg: CNNAblationConfig, epochs_override: int | None = None) -> dict:
    ...
    if len(train_ds) == 0:
        logger.error(
            "No training samples found. Run data pipeline first: just generate-data"
        )
        return {}
    ...
    for epoch in range(1, cfg.epochs + 1):
        with logger.contextualize(epoch=epoch, run=cfg.run_id):
            ...
            if early_stopper.step(val_f1, model):
                logger.info(
                    "Early stop at epoch {} — no val_f1 improvement for {} epochs",
                    epoch,
                    cfg.patience,
                )
                break
    ...
    _save_curve(
        ...
        f"Loss Curve — {cfg.run_id}",
        "Loss",
        fig_out / "loss_curve.png",
    )
    _save_curve(
        {"train": train_f1s, "val": val_f1s},
        f"Macro-F1 Curve — {cfg.run_id}",
        "Macro-F1",
...
def compile_ablation_summary() -> None:
    ...
    logger.info("\n" + "=" * 70)
    logger.info("CNN ABLATION SUMMARY")
    logger.info("=" * 70)
    ...
    for r in sorted(
        results,
        key=lambda x: x["test_f1"] if isinstance(x["test_f1"], float) else -1,
        reverse=True,
    ):
        desc = (
            r["description"][:50] + "…"
        ...
        logger.info(
            f"{r['run_id']:<12} {str(r['val_f1']):>8} {str(r['test_f1']):>8} "
...
def main() -> None:
    ...
    if args.list:
        ...
        for rid, cfg in ALL_RUNS.items():
            logger.info(f"{rid:<12} {cfg.description}")
        return
