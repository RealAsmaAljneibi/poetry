...
from src.config import arapoem_genre_config
...
def _load_mel(
    path: str, sr: int = 16_000, n_mels: int = 128, max_sec: float = 8.0
) -> torch.Tensor:
    try:
        ...
        if mel_t.shape[1] < target_frames:
            mel_t = torch.nn.functional.pad(mel_t, (0, target_frames - mel_t.shape[1]))
        else:
...
def eval_whisper_finetuned(records: list[dict], device: str) -> dict:
    ...
    base_model = WhisperForConditionalGeneration.from_pretrained(
        base_name, local_files_only=True
    )
...
def print_comparison(results: list[dict], baseline_csv: Path) -> None:
    ...
    for r in results:
        ...
        if "emotion_partial_credit" in r:
            ...
            sign = "+" if delta >= 0 else ""
            logger.info(
                f"  Emotion PC  : {r['emotion_partial_credit']:.4f}  "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate trained model checkpoints (Metrics v3)"
    )
    parser.add_argument(
        "--only",
        choices=["arapoem", "audio", "whisper"],
        default=None,
        help="Run only one model evaluation",
    )
    parser.add_argument(
        "--skip-whisper",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-audio", action="store_true", help="Skip Audio CNN inference"
    )
