...
# ── Paths ─────────────────────────────────────────────────────────────────────
EMOTION_CHECKPOINT = Path("outputs/models/arapoem_emotion/arapoem_emotion_text_window3_best.pt")
...
def precompute_emotion_embeddings(device: torch.device, batch_size: int = 64) -> None:
    ...
    for split, jsonl_path in DATA_SPLITS.items():
        ...
        with open(out_path, "wb") as f:
            pickle.dump(poems, f)
        logger.success(f"  {split}: {len(records)} clips, {len(poems)} poems → {out_path}")
...
class EmotionSequenceDataset(Dataset):
    def __init__(self, split: str, use_pos_features: bool = False):
        ...
        for pid, clips in poems.items():
            ...
            if use_pos_features:
                pos_feat = np.array([
                    [c["pos"] / max(1, n - 1), float(c["pos"] == 0), float(c["pos"] == n - 1)]
                    for c in clips
    ...
    def __len__(self):  return len(self.poems)
    def __getitem__(self, idx): return self.poems[idx]
...
def collate_poems(batch: list[dict]) -> dict:
    ...
    return {"cls": padded_cls, "labels": padded_labels, "lengths": lengths,
...
class EmotionBiLSTM(nn.Module):
    def __init__(self, input_dim: int = 768, lstm_hidden: int = 256,
                 num_layers: int = 1, num_classes: int = NUM_CLASSES, dropout: float = 0.4):
        super().__init__()
    ...
    def forward(self, cls_seq: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        packed       = pack_padded_sequence(cls_seq, lengths.cpu(), batch_first=True, enforce_sorted=False)
...
def eval_epoch(model, loader, criterion, device):
    ...
    poem_f1 = f1_score(poem_true_list, poem_pred_list, average="macro", zero_division=0) if poem_true_list else 0.0
    return total_loss / max(len(loader), 1), clip_f1, all_preds, all_true, poem_f1, poem_true_list, poem_pred_list
...
def main(args: argparse.Namespace) -> None:
    ...
    train_loader = DataLoader(train_ds, batch_size=4, shuffle=True,  collate_fn=collate_poems, num_workers=0)
    val_loader   = DataLoader(val_ds,   batch_size=4, shuffle=False, collate_fn=collate_poems, num_workers=0)
    test_loader  = DataLoader(test_ds,  batch_size=4, shuffle=False, collate_fn=collate_poems, num_workers=0)

    # Class weights (sqrt for extreme imbalance)
    all_train_labels = [lbl for ds_item in train_ds for lbl in ds_item["label_seq"].tolist()]
    ...
    logger.info(f"EmotionBiLSTM: {n_params:,} params, input_dim={input_dim}, classes={NUM_CLASSES}")

    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    total_steps = args.epochs * len(train_loader)
    warmup = max(1, int(0.1 * total_steps))
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=1e-3, total_steps=total_steps, pct_start=warmup/total_steps,
    )
    ...
    stopper   = EarlyStopper(patience=args.patience, output_dir=OUTPUT_DIR, model_tag=run_tag)

    best_val_f1 = 0.0
    for epoch in range(args.epochs):
        tr_loss, tr_acc = train_epoch(model, train_loader, optimizer, scheduler, criterion, device)
        val_loss, val_f1, _, _, val_poem_f1, _, _ = eval_epoch(model, val_loader, criterion, device)
        logger.info(
        ...
        tb_logger.log_epoch(
            ...
            val_loss=val_loss, val_f1=val_f1, val_acc=0.0,
        )
    ...
    test_loss, test_f1, test_preds, test_true, poem_f1, poem_true, poem_pred = eval_epoch(
    ...
    ax.set_ylabel("True")
    ax.set_title(f"Test Confusion — EmotionBiLSTM {'+ pos' if args.pos_features else ''} | F1={test_f1:.3f}")
    cm_path = REPORT_DIR / f"emotion_bilstm{'_pos' if args.pos_features else ''}_confusion.png"
    ...
    json_path = REPORT_DIR / f"emotion_bilstm{'_pos' if args.pos_features else ''}_eval.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pos-features", action="store_true",
