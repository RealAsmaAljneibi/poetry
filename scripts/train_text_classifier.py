...
Course techniques applied:
  Week 2 — LR warmup + cosine decay            (get_scheduler)
  Week 3 — Focal loss, early stopping           (FocalLoss, EarlyStopper)
  Week 4 — Discriminative LR, gradual unfreezing (get_optimizer, unfreeze_next_layer_group)
...
Usage:
    ...
    # Transfer-learning: freeze encoder, train head only (Lab 03 strategy)
...
from src.training.trainer import (
    ...
    get_optimizer, get_scheduler, get_one_cycle_scheduler,
    unfreeze_next_layer_group, set_seed,
)
from src.training.sanity import run_all_checks


# ── Focal Loss (Week 3: handles Ghazal>>Tareef imbalance) ────────────────────
    ...
    def __init__(
        ...
        for pid, clips in poem_records.items():
            ...
            for i, rec in enumerate(clips):
                ...
                self.samples.append({"text": text, "label": label_id, "poem_id": poem_id})

        if context_window > 1:
            import numpy as _np
            logger.info(
                f"Sliding window={context_window}: "
                f"avg_clips_per_window={_np.mean(window_sizes):.2f}  "
...
def train_epoch(
    model, loader, optimizer, scheduler, criterion, device, tb_logger,
    global_step: int, log_every: int, accum_steps: int = 1,
) -> tuple[float, float, int]:
    """
    Gradient accumulation (SSL lab, Week 4): accumulate gradients over
    ...
    for i, batch in enumerate(loader):
        ...
        if is_accum_boundary or is_last_batch:
            # Gradient clipping (debugging lab: prevents exploding gradients)
...
def poem_aggregate_eval(
    ...
    for pid, probs_list in poem_probs.items():
        avg = [sum(p[i] for p in probs_list) / len(probs_list) for i in range(len(probs_list[0]))]
...
def main(args):
    ...
    unfreeze_after = args.unfreeze_after   # epoch index (0-based) to unfreeze last 2 layers
    ...
    if args.loss == "focal":
        cfg = cfg.model_copy(update={"use_focal_loss": True,
                                      "focal_gamma": args.focal_gamma})
    elif args.loss == "ce":
        cfg = cfg.model_copy(update={"use_focal_loss": False})

    # Emotion merge profile
    emotion_merge_profile: str = args.emotion_merge_profile if args.task == "emotion_text" else "none"

    # Pydantic already validated all field constraints at construction time
    logger.info(f"Config: {cfg.model_name} | task={cfg.task} | epochs={cfg.epochs} | "
    ...
    if cfg.task == "genre":
        ...
        logger.info(f"Emotion merge profile '{emotion_merge_profile}': "
    ...
    model_cfg = _AutoConfig.from_pretrained(cfg.model_name, local_files_only=True)
    model_cfg.num_labels = num_classes   # must be set in config when passing config= explicitly
    if cfg.dropout > 0.0:
    ...
    # ── Unfreeze-last-n: static partial fine-tuning (Knob 4 / Lab 03) ───────
    # --unfreeze-last-n N  freezes all encoder layers except the last N.
    # N=0: head-only; N=2/3: partial; N=full/None: all trainable (default).
    ...
    if unfreeze_n is not None and unfreeze_n != "full":
        ...
        logger.info(f"Static partial fine-tune: unfreeze_last_n={n_layers} | "
    ...
    # ── Frozen-encoder / head-only mode (Lab 03: transfer learning) ──────────
    unfreeze_params: list = []   # params pre-registered at lr=0 for progressive unfreeze
    if freeze_encoder:
        # Freeze everything in the base encoder
        for name, param in model.named_parameters():
    ...
    train_ds = NabatiTextDataset(cfg.train_jsonl, tokenizer, cfg.task, cfg.max_seq_len, cw,
                                  emotion_merge_profile=emotion_merge_profile)
    val_ds   = NabatiTextDataset(cfg.val_jsonl,   tokenizer, cfg.task, cfg.max_seq_len, cw,
                                  emotion_merge_profile=emotion_merge_profile)
    test_ds  = NabatiTextDataset(cfg.test_jsonl,  tokenizer, cfg.task, cfg.max_seq_len, cw,
                                  emotion_merge_profile=emotion_merge_profile)
    ...
    # ── Class weights (inverse frequency, from TL lab) ───────────────────────
    ...
    # Course note: sqrt weights preferred over raw inverse-frequency for high imbalance.
    ...
    if raw_ratio > 20.0:
        ...
        logger.info(f"Class weight ratio {raw_ratio:.1f}x > 20x — applying sqrt to moderate imbalance")
    ...
    logger.info(f"Class weights: min={class_w.min():.2f}, max={class_w.max():.2f}, ratio={class_w.max()/class_w.min():.1f}x")

    # ── Loss function ─────────────────────────────────────────────────────────
    if cfg.use_focal_loss:
        criterion = FocalLoss(gamma=cfg.focal_gamma, weight=class_weights).to(device)
        logger.info(f"Using FocalLoss(γ={cfg.focal_gamma}) + balanced class weights + "
                    f"label_smoothing={cfg.label_smoothing}")
    else:
        criterion = nn.CrossEntropyLoss(weight=class_weights,
                                         label_smoothing=cfg.label_smoothing)
        logger.info(f"Using CrossEntropyLoss + balanced class weights + "
                    f"label_smoothing={cfg.label_smoothing}")

    # ── Optimizer (discriminative LR, Week 4) ────────────────────────────────
    if freeze_encoder:
        # Head-only optimizer: flat lr=1e-3, weight_decay=0.01 (Lab 03)
        ...
        if unfreeze_params:
            ...
            logger.info(f"Head-only optimizer: AdamW lr={head_lr:.2e} weight_decay=0.01")
    else:
    ...
    tb_logger.log_hparams(
        ...
        metric_dict={"hparam/val_macro_f1": 0.0},  # placeholder, updated after training
    )

    # ── Pre-training sanity checks (Week 5 debugging lab) ────────────────────
    ...
    early_stopper = EarlyStopper(cfg.patience, cfg.output_dir, f"{model_tag}_{cfg.task}")
    global_step   = 0
    best_val_f1   = 0.0
    ...
    for epoch in range(cfg.epochs):
        ...
        epoch_logger = logger.bind(epoch=epoch + 1, total_epochs=cfg.epochs, task=cfg.task)
        ...
        # Gradual unfreezing (Week 4): unfreeze one layer group every N epochs
        ...
        # Log gradient histograms (Week 5 debugging lab)
        ...
        if (epoch + 1) % 5 == 0:
            fig = make_confusion_figure(
                val_true, val_preds, class_names,
    ...
    test_loss, test_f1, test_acc, test_preds, test_true, test_probs, test_poem_ids = eval_epoch(
    ...
    poem_f1, poem_acc = poem_aggregate_eval(test_probs, test_true, test_poem_ids, class_names)
    logger.success(
    ...
    # ── Final confusion matrix (saved to disk) ────────────────────────────────
    report_dir = Path("outputs/reports")
    figures_dir = Path("outputs/figures")
    report_dir.mkdir(parents=True, exist_ok=True)
    figures_dir.mkdir(parents=True, exist_ok=True)
    # Use run_id for unique filenames; fall back to default for backwards compat
    _run_id_safe = (getattr(args, "run_id", None) or
                    f"{model_tag}_{cfg.task}_{emotion_merge_profile}").replace("/", "_")
    fig_path = figures_dir / f"emotion_confusion_{_run_id_safe}.png"
    fig = make_confusion_figure(
        test_true, test_preds, class_names,
    ...
    # ── Secondary metrics: Top-2 accuracy + ECE ──────────────────────────────
    # Top-2 accuracy: correct if true label is in top-2 predictions
    top2_correct = sum(
        1 for prob, true in zip(test_probs, test_true)
    ...
    correctness = np.array([int(p == t) for p, t in zip(test_preds, test_true)], dtype=float)
    ece = 0.0
    ...
    # ── Save JSON report ──────────────────────────────────────────────────────
    run_id = getattr(args, "run_id", None) or f"{model_tag}_{cfg.task}_{emotion_merge_profile}"
    report = {
        "run_id":          run_id,
        "task":            cfg.task,
        "model":           cfg.model_name,
    ...
    json_report_path = Path(getattr(args, "report_path", None) or
                            report_dir / f"emotion_eval_{emotion_merge_profile}.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", choices=["arapoem", "mbert"], default="arapoem",
        help="arapoem = faisalq/bert-base-arapoembert  |  mbert = bert-base-multilingual-cased"
    )
    parser.add_argument(
        "--task", choices=["genre", "emotion_text"], default="genre",
        help="Which label column to predict."
    )
    parser.add_argument(
        "--freeze-encoder", action="store_true",
        help="Freeze BERT encoder; train only a 768→256→11 head with dropout=0.5 (Lab 03)."
    )
    parser.add_argument(
        "--unfreeze-after", type=int, default=None, metavar="EPOCH",
    ...
    parser.add_argument(
        "--context-window", type=int, default=1, metavar="N",
    ...
    parser.add_argument(
        "--loss", choices=["ce", "focal"], default=None,
    ...
    parser.add_argument(
        "--unfreeze-last-n", default=None, dest="unfreeze_last_n",
    ...
    parser.add_argument(
        "--report-path", default=None, dest="report_path",
