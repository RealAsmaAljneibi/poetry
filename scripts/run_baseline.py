...
Baselines implemented:
  ...
  │2 │ Majority-class              │ Emotion    │
  │3 │ TF-IDF + LogReg (char n-gram│ Genre      │
  │4 │ TF-IDF + LogReg (char n-gram│ Emotion    │
  │5 │ MFCC mean + SVM (RBF)       │ Emotion    │
  │6 │ mBERT fine-tuned            │ Genre      │
  └──┴─────────────────────────────┴────────────┘
...
def evaluate(
    ...
    emotion_partial_scores: list[float] | None = None,
) -> dict:
    """Full evaluation suite — Macro-F1, Weighted-F1, Cohen's κ, and optional partial-credit."""
    ...
    if emotion_partial_scores is not None:
        ...
        logger.info(
            f"  Emotion PC  : {mean_pc:.4f}  (partial-credit: 1.0/0.65/0.45/0.30/0.20)"
        )
...
def save_confusion_matrix(
    y_true, y_pred, class_names: list[str], title: str, fname: str
) -> None:
    ...
    fig, ax = plt.subplots(
        figsize=(max(10, len(class_names)), max(8, len(class_names) - 1))
    )
    sns.heatmap(
        cm_norm,
        annot=cm,
    ...
    for _ in range(3):
        ...
        if cm_off[idx] == 0:
            break
        logger.info(
            f"  Confused: '{class_names[idx[0]][:30]}' "
...
def run_majority(
    train: pd.DataFrame, test: pd.DataFrame, task_col: str, task_name: str
) -> dict:
...
def run_tfidf_logreg(
    ...
    # Character n-grams (2–5) — better for Arabic morphology
    vec = TfidfVectorizer(
        analyzer="char_wb",
        ngram_range=(2, 5),
        max_features=15_000,
    ...
    save_confusion_matrix(
        y_test, y_pred, classes, f"TF-IDF + LogReg | {task_name}", cm_fname
    )
...
def run_mfcc_svm(train: pd.DataFrame, test: pd.DataFrame) -> dict:
    ...
    clf = SVC(
        kernel="rbf", class_weight="balanced", C=10, gamma="scale", random_state=SEED
    )
    t0 = time.time()
    clf.fit(X_train, y_train)
    ...
    y_pred = clf.predict(X_test)
    save_confusion_matrix(
        y_test, y_pred, classes, "MFCC + SVM | Emotion (audio)", "cm_mfcc_svm.png"
    )
...
def run_mbert(train: pd.DataFrame, val: pd.DataFrame, test: pd.DataFrame) -> dict:
    """
    ...
    val_c["label"] = le.transform(
        val_c[task_col].map(lambda x: x if x in le.classes_ else le.classes_[0])
    )
    test_c["label"] = le.transform(
        test_c[task_col].map(lambda x: x if x in le.classes_ else le.classes_[0])
    )
    ...
    save_confusion_matrix(
        y_true, y_pred, classes, "mBERT fine-tuned | Genre", "cm_mbert_genre.png"
    )
...
def main() -> None:
    ...
    # 3. TF-IDF + LR — Genre
    results.append(
        run_tfidf_logreg(train, test, "genre_en", "Genre", "cm_tfidf_genre.png")
    )
    ...
    logger.info("═" * 65)
    logger.info("\nInterpretation:")
    logger.info("  • Majority-class Macro-F1 ≈ 0.03–0.05 → confirms imbalance")
    logger.info("  • TF-IDF shows what pure text structure gives")
    logger.info("  • mBERT sets the bar that AraPoemBERT must beat")
    logger.info("  • MFCC+SVM sets the bar that Emotion1DCNN must beat")
    logger.info("  • Arousal majority ≈ 0.33 → our MLP (0.648) is ×2 above floor")
