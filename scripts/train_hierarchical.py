...
class PoemSequenceDataset(Dataset):
    ...
    def __init__(self, split: str, use_pos_features: bool = False):
        ...
        logger.info(f"PoemSequenceDataset [{split}]: {len(self.poems)} poems, "
...
def train_epoch(model, loader, optimizer, scheduler, criterion, device) -> tuple[float, float]:
    model.train()
    total_loss = 0.0
    n_correct = n_total = 0
...
def main(args):
    ...
    train_loader = DataLoader(
        ...
        collate_fn=collate_poems, num_workers=0,
    )
    ...
    scheduler = LambdaLR(
        optimizer,
        lr_lambda=lambda s: min((s+1)/warmup_steps, 1.0) if s < warmup_steps
                   else max(0.0, 0.5*(1 + math.cos(math.pi*(s-warmup_steps)/max(total_steps-warmup_steps,1)))),
    )
    logger.info(f"AdamW lr={args.lr:.1e}  epochs={args.epochs}  patience={args.patience}")
    ...
    # ── Training loop ─────────────────────────────────────────────────────────
    logger.info("=" * 60)
    logger.info(f"Hierarchical BiLSTM | {args.epochs} epochs | "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pos-features", action="store_true",
        help="Strategy 2: prepend [position_ratio, is_first, is_last] to each CLS vector."
    )
    parser.add_argument("--lstm-hidden",  type=int,   default=256,  help="BiLSTM hidden size per direction")
