...
Course: Week 5 Lab — Generative Models (05_lab_generative_models/02_vae.ipynb)
  "Train a VAE on mel spectrograms of audio recordings.
   Sample from learned latent distribution (near minority class centroids)
   to generate new synthetic audio spectrograms."
...
class MelDataset(Dataset):
    ...
    def __init__(self, jsonl_path: Path, augment: bool = False):
        ...
        logger.info(f"MelDataset: {len(self.samples)} clips — "
...
class ConditionalVAE(nn.Module):
    ...
    Architecture mirrors the course VAE lab with two extensions:
      1. Conditional: class label injected at encoder input and decoder input
      2. β-VAE: β controls KL weight (β=1 = standard VAE)
    """
    ...
    def __init__(self, input_dim: int, latent_dim: int, num_classes: int,
        ...
        # Weight init (Kaiming for ReLU layers, Xavier for output)
...
def train_vae(
    ...
    logger.info("=" * 55)
    logger.info(f"cVAE training: {epochs} epochs, β={beta}, latent_dim={model.latent_dim}")
    logger.info("=" * 55)
...
def generate_for_minority_classes(
    ...
    with torch.no_grad():
        for cls_id in range(NUM_CLASSES):
            ...
            logger.info(f"  Generating {n_generate} samples for '{emotion_name}' "
                        f"(real={real_count}, target={target_per_class})")

            y_oh = F.one_hot(
                torch.tensor([cls_id] * n_generate, device=device),
                NUM_CLASSES
            ).float()
    ...
    logger.success(f"Generated {len(records)} synthetic mel spectrograms → {output_dir}")
...
def main(args: argparse.Namespace) -> None:
    ...
    logger.info(f"cVAE: {total_params:,} params | input_dim={INPUT_DIM} | "
    ...
    generate_for_minority_classes(
        model, train_ds, device, OUTPUT_DIR,
        target_per_class=args.target_per_class,
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train conditional VAE for mel augmentation")
