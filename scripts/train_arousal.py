...
def extract_features(
    audio_path: str, sr: int = 16000, max_sec: int = 30, n_mfcc: int = 13
) -> np.ndarray | None:
    """
...
def build_feature_cache(
    jsonl_path: Path, sr: int, max_sec: int, n_mfcc: int
) -> dict[str, np.ndarray]:
...
def load_split(
    ...
    for line in open(jsonl_path):
        ...
        y_rows.append(label)
    logger.info(
        f"{jsonl_path.name}: {len(X_rows)} samples loaded, {skipped} skipped (no audio/label)"
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    if use_cache:
        ...
        for split_path in [cfg.train_jsonl, cfg.val_jsonl, cfg.test_jsonl]:
            feature_cache.update(
                build_feature_cache(
                    split_path, cfg.sample_rate, cfg.max_audio_sec, cfg.n_mfcc
                )
            )
        ...
        logger.info(
            f"Feature cache saved: {len(feature_cache)} clips → {FEATURE_CACHE}"
        )
    ...
    logger.info(
        f"Feature dim: {input_dim} | train={len(X_train)} val={len(X_val)} test={len(X_test)}"
    )
    ...
    logger.info(
        f"Sanity: initial loss={init_loss:.4f}  expected≈log({N_CLASSES})={expected:.4f}  "
    ...
    for epoch in range(cfg.epochs):
        epoch_logger = logger.bind(
            epoch=epoch + 1, total_epochs=cfg.epochs, task="arousal"
        )
