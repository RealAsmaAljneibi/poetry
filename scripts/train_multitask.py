...
Multi-task learning: joint genre + emotion classification.
...
Rationale:
  ...
  gradient signal — a classic multi-objective technique.
...
def train_epoch(
    ...
    for i, batch in enumerate(loader):
        ...
        loss = (
            g_weight * g_crit(g_logits, g_labels)
...
def eval_epoch(model, loader, g_crit, e_crit, g_weight, e_weight, device, n_g, n_e):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            g_logits, e_logits = model(ids, mask)
            loss = g_weight * g_crit(g_logits, g_labels) + e_weight * e_crit(
                e_logits, e_labels
            )
...
def main(args):
    ...
    logger.info(
        f"Multi-task: genre={n_genre} classes | emotion={n_emotion} classes "
        f"(profile={emotion_merge_profile!r})"
    )

    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, local_files_only=True)
    model = MultitaskModel(MODEL_NAME, n_genre, n_emotion, dropout=args.dropout).to(
        device
    )
    ...
    optimizer.add_param_group(
        {
            "params": list(model.genre_head.parameters()),
            "lr": args.lr,
            "weight_decay": args.weight_decay,
        }
    )
    ...
    logger.info(
        f"Training: {args.epochs} epochs | genre_w={args.genre_weight} | "
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(
            f"Epoch {epoch + 1:3d}/{args.epochs} | loss={tr_loss:.4f} | "
    ...
    fig_e = make_confusion_figure(
        e_true,
        e_preds,
        emotion_classes,
        f"Multitask Emotion (clip Macro-F1={test_e_f1:.3f})",
    )
    fig_g = make_confusion_figure(
        g_true,
        g_preds,
        GENRE_CLASSES,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--focal", action="store_true", help="Use FocalLoss for emotion head"
    )
