...
def compile_results(reports_dir: Path) -> dict[str, list[dict[str, Any]]]:
    """Compile all results into categories. Returns category -> list of technique dicts."""
    techniques = {
            ...
            techniques["asr"].append(
                {
                    ...
                    "poem_f1": None,
                    "status": "adopted",
                    "notes": "WER=0.245, CER=0.081, soft_CER=0.075 (baseline)",
                }
            )
                ...
                techniques["asr"].append(
                    {
                        ...
                        "poem_f1": None,
                        "status": "negative",
                        "notes": "All runs worse: WER=0.589, CER=0.224, soft_CER=0.209",
                    }
                )
...
def generate_json_summary(
    ...
    summary["summary_stats"] = {
        ...
        "total_adopted": sum(
            1 for v in techniques.values() for t in v if t["status"] == "adopted"
        ),
        "total_negative": sum(
            1 for v in techniques.values() for t in v if t["status"] == "negative"
        ),
        "total_baseline": sum(
            1 for v in techniques.values() for t in v if t["status"] == "baseline"
        ),
        "total_documented": sum(
            1 for v in techniques.values() for t in v if t["status"] == "documented"
        ),
    }
...
def main() -> None:
    logger.add("logs/comprehensive_ablation.log", rotation="10 MB")
    ...
    md_output = reports_dir / "comprehensive_techniques_table.md"
    with open(md_output, "w", encoding="utf-8") as f:
        f.write(md_table)
    logger.info(f"Wrote markdown table: {md_output}")

    json_summary = generate_json_summary(techniques)
    json_output = reports_dir / "comprehensive_techniques_table.json"
    ...
    logger.info("=== SUMMARY ===")
    logger.info(
        f"Total techniques: {json_summary['summary_stats']['total_techniques']}"
    )
