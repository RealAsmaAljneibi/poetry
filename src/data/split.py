...
def split_dataset(
    ...
    with open(input_jsonl, encoding="utf-8") as f:
        for line in f:
            rec = PoetrySample.model_validate_json(line).model_dump()
    ...
    poets_sorted = sorted(
        poet_to_records, key=lambda poet: (-len(poet_to_records[poet]), poet)
    )
    ...
    genre_split: dict[str, dict[str, int]] = defaultdict(
